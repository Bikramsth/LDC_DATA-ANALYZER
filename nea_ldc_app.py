...
with t1:
    ...
    if selected_day:
        ...
        if not df_all.empty:
            ...
            with st3:
                # 🚀 EXPORT TAB MODIFICATION ONLY
                df_exp = df_all[df_all['parameter_name'].str.startswith('ZONE_EXPORT_') | 
                                df_all['parameter_name'].str.startswith('ZONE_IMPORT_') | 
                                (df_all['parameter_name'] == 'SUMMARY_TOTAL_EXPORT')].copy()
                
                if not df_exp.empty:
                    df_exp['parameter_name'] = df_exp['parameter_name'].str.replace('ZONE_EXPORT_', '')
                    df_exp['parameter_name'] = df_exp['parameter_name'].str.replace('ZONE_IMPORT_', '')
                    ...
                    # Target Katiya and Muzaffarpur explicitly
                    muj_cols = [c for c in df_p_exp.columns if 'MUJAFFARPUR' in c.upper() or 'MUZAFFARPUR' in c.upper()]
                    ...
                    # "Katiya line is both export and import line also combine with other plot."
                    for kc in katiya_cols:
                        df_p_exp['Total EXPORT'] += df_p_exp[kc] 
                        df_p_exp.drop(columns=[kc], inplace=True)
                        
                    # "if the value is negative plot that in negative axis, if positive plot in negative axis"
                    for c in df_p_exp.columns:
                        df_p_exp[c] = -df_p_exp[c].abs()
                        
                    # Filter out purely zero columns to keep it clean
                    v_cols = [c for c in df_p_exp.columns if df_p_exp[c].min() < 0]
